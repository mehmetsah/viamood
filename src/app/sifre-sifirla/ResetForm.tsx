...
export function ResetForm({ token }: { token: string }) {
  ...
  const [state, formAction, pending] = useActionState(
    async (_prev: ActionResult | null, formData: FormData) => resetPasswordAction(formData),
    initialState,
  );

  const basarili = state?.success === true;

  // Şifre değişti → giriş sayfasına yolla. Kısa bekleme, kullanıcı onay mesajını görsün.
  useEffect(() => {
    if (!basarili) return;
