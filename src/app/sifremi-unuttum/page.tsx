...
export default function SifremiUnuttumPage() {
  const [state, formAction, pending] = useActionState(
    async (_prev: ActionResult | null, formData: FormData) => requestPasswordResetAction(formData),
    initialState,
  );
  ...
  const mesaj = basarili
    ? (state.data as { message?: string } | undefined)?.message
    : undefined;
      ...
      <div className="w-full max-w-md">
        ...
        {basarili ? (
          ...
          >
            ...
            <Link
              href="/auth/sign-in"
              className="text-center text-sm text-neutral-600 hover:underline mt-1"
            >
              Giriş sayfasına dön
            </Link>
          </form>
