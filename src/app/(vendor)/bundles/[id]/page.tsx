...
import { BundleManageClient } from './BundleManageClient';
...
export default async function BundleDetailPage({ params }: PageProps) {
  ...
  const [bundle] = await db
    .select()
    .from(productBundles)
    .where(and(eq(productBundles.id, id), eq(productBundles.vendorId, membership.vendorId)))
    .limit(1);
  ...
  const isPushed = !!bundle.shopifyProductId;
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 mt-2 mb-8">
        <div>
          ...
          <p className="text-neutral-500 text-sm font-mono mt-1">{bundle.sku}</p>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <table className="w-full text-sm">
              <thead className="text-xs uppercase tracking-wider text-neutral-500">
                <tr>
                  ...
                  <th className="text-right px-4 py-2">Tekil Kargo</th>
                ...
                {components.map((c) => (
                  <tr key={c.id}>
                    ...
                    <td className="px-4 py-3 text-right font-mono text-neutral-600">
                      {formatTL(Number(c.shippingSnapshot ?? 0n) * c.quantity)}
                    </td>
          ...
          {/* Stok yönetimi + status + Shopify push */}
          <BundleManageClient
            bundleId={bundle.id}
        ...
        <aside className="space-y-4">
          {metrics && (
            <section className="bg-[var(--color-brand-ink)] text-white rounded-xl p-6">
              ...
              <div className="space-y-2 text-sm">
                ...
                {metrics.discountCents > 0 && (
                  <Row
                    label={`İndirim (%${(metrics.discountPercent * 100).toFixed(0)})`}
                    value={formatTL(metrics.discountCents)}
                    negative
                  />
                )}
                <div className="border-t border-white/20 my-2" />
                <Row label="Tekil kargo (toplam)" value={formatTL(metrics.individualShippingCents)} />
                <Row label="Set kargo" value={formatTL(metrics.bundleShippingCents)} />
                <Row
                  label="Kargo tasarrufu"
                  value={formatTL(metrics.shippingSavingsCents)}
                  positive={metrics.shippingSavingsCents > 0}
                />
                <div className="border-t border-white/20 my-2" />
                <Row
                  label="Tekil satış karı"
                  value={formatTL(metrics.profitIfSoldIndividuallyCents)}
                />
                <Row
                  label="Set satış karı"
                  value={formatTL(metrics.profitIfSoldAsBundleCents)}
                  bold
                ...
                <Row
                  label="Fark"
                  value={formatTL(metrics.profitDifferenceCents)}
          ...
          {metrics && metrics.componentProfitShare.length > 0 && (
            <section className="bg-white rounded-xl border p-5">
              ...
              <p className="text-xs text-neutral-500 mb-3">Maliyet oranına göre dağıtım</p>
